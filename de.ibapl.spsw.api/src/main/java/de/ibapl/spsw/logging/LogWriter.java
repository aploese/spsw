...
public class LogWriter {
     ...
     * @param ascii If true write bytes in their ASCII representation otherwise
     * format hexadecimal.
    ...
    public void explainRead(Instant ts, String fmt, Object... args) {
        log.append(formatTs(ts)).append("ExR\t\"").printf(fmt, args).println("\"");
        log.flush();
    }

    public void explainRead(Instant ts, Throwable t) {
        log.append(formatTs(ts)).append("ExR\t");
        t.printStackTrace(log);
        log.flush();
    }

    public void explainWrite(Instant ts, String fmt, Object... args) {
        log.append(formatTs(ts)).append("ExW\t\"").printf(fmt, args).println("\"");
        log.flush();
    }

    public void explainWrite(Instant ts, Throwable t) {
        log.append(formatTs(ts)).append("ExW\t");
        t.printStackTrace(log);
        log.flush();
    }
