...
JNIEXPORT jint JNICALL JNI_OnLoad(JavaVM *jvm, void *reserved) {
	...
	genericTermiosSerialPortSocket = (*env)->FindClass(env,
...
static void setLineStatus(JNIEnv *env, jobject object, jboolean enabled,
		int bitMask) {
	int fd = (*env)->GetIntField(env, object, spsw_fd);
	int lineStatus;
	if (ioctl(fd, TIOCMGET, &lineStatus) != 0) {
...
(JNIEnv *env, jobject object, jint baudRate) {
	...
	if (cfsetspeed(&settings, baudRateValue) < 0) {
		throw_ClosedOrNativeException(env, object, "setBaudrate: Baudrate of in and output mismatch");
		return;
	}
...
(JNIEnv *env, jobject object, jint dataBits) {
	...
	if ((settings.c_cflag & CSIZE) != (settingsRead.c_cflag & CSIZE)) {
		throw_Illegal_Argument_Exception(env, "setDataBits: DataBits where not set correctly!");
	}
}
...
(JNIEnv *env, jobject object, jint stopBits) {
	...
	if ((settings.c_cflag &= ~CSTOPB) != (settingsRead.c_cflag &= ~CSTOPB)) {
		throw_Illegal_Argument_Exception(env, "setStopBits: StopBits where not set correctly!");
	}
}
...
JNIEXPORT jint JNICALL Java_de_ibapl_spsw_provider_GenericTermiosSerialPortSocket_getInterByteReadTimeout(
	...
	if (tcgetattr(fd, &settings) != 0) {
		throw_ClosedOrNativeException(env, object,
				"getInterByteReadTimeout tcgetattr");
		return -1;
	}

	if (settings.c_cc[VMIN] != 1) {
		throw_SerialPortException_NativeError(env,
				"getInterByteReadTimeout Wrong Timeout settings");
		return -1;
...
JNIEXPORT jint JNICALL Java_de_ibapl_spsw_provider_GenericTermiosSerialPortSocket_getOverallReadTimeout(
		JNIEnv *env, jobject object) {
	int pollReadTimeout = (*env)->GetIntField(env, object,
			spsw_pollReadTimeout);
...
JNIEXPORT jint JNICALL Java_de_ibapl_spsw_provider_GenericTermiosSerialPortSocket_getOverallWriteTimeout(
		JNIEnv *env, jobject object) {
	int pollWriteTimeout = (*env)->GetIntField(env, object,
			spsw_pollWriteTimeout);
